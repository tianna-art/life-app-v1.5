 ...
 */
describe('TruncatedLogRow', () => {
  it('shows the category and the moment tags', () => {
    const screen = render(<TruncatedLogRow entry={base} onPress={jest.fn()} />);
  ...
  it('shows what the person wrote when they wrote something', () => {
    const screen = render(
      <TruncatedLogRow entry={{ ...base, optionalAnswer: '声が震えていた' }} onPress={jest.fn()} />
    );
    expect(screen.getByText(/声が震えていた/)).toBeTruthy();
  });
